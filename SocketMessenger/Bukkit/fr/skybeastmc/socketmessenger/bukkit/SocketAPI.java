...
public class SocketAPI {
	 ...
	 *
	 * @param channel
	 *            - The channel to send the Data trough
	 * @param data
	 *            - The Data to send
	 */
	 ...
	 *            - The Data to send
	 */
	public static void sendDataToServer(String channel, byte[] data) {
		SocketManager.sendCommand(Command.SEND_DATA, channel, data);
	}
	...
	public static void broadcastToServer(String msg) {
		SocketManager.sendCommand(Command.BROADCAST, msg);
	}
	 ...
	 *
	 * @param server
	 *            - The server to forward the Data to
	 * @param data
	 *            - The Data to forward
	 */
	 ...
	 *            - The Data to forward
	 */
	public static void forwardDataToServer(String server, byte[] data) {
		SocketManager.sendCommand(Command.FORWARD_DATA, server, data);
	}

	/**
	 * Connects a specified player to a specified server on the BungeeCord
	 * network.
	 *
	...
	public static void connectPlayerToServer(String player, String server) {
		SocketManager.sendCommand(Command.CONNECT, player, server);
	}

	/**
	 * Requests the player count on the BungeeCord network.
	 *
	 * The count is sent back as a Data object trough ReceivedDataEvent on
	 * channel 'PlayerCount'. Retrieve the count (int) by calling
	 * getInt("playerCount") on the received data object.
	 */
	public static void requestPlayerCount() {
		SocketManager.sendCommand(Command.PLAYER_COUNT);
	}

	/**
	 * Requests the list of online players on the BungeeCord network.
	 *
	 * The list is sent back as a Data object trough ReceivedDataEvent on
	 * channel 'PlayerList'. Retrieve the list (ArrayList<String>) by calling
	 * getObject("playerList") on the received data object and casting the
	 * result to an ArrayList<String>.
	 */
	public static void requestPlayerList() {
		SocketManager.sendCommand(Command.PLAYER_LIST);
	}
	 ...
	 *
	 * The list is sent back as a Data object trough ReceivedDataEvent on
	 * channel 'ServerList'. Retrieve the list (ArrayList<String>) by calling
	 * getObject("serverList") on the received data object and casting the
	 * result to an ArrayList<String>.
	 */
	public static void requestServerList() {
		SocketManager.sendCommand(Command.GET_SERVERS);
	}
	...
	public static void messagePlayer(String player, String message) {
		SocketManager.sendCommand(Command.MESSAGE, player, message);
	}

	/**
	 * Requests the name of this server on the BungeeCord network (as configured
	 * in config.yml).
	 *
	 * The name is sent back as a Data object trough ReceivedDataEvent on
	 * channel 'ServerName'. Retrieve the name (String) by calling
	 * getString("serverName") on the received data object.
	 */
	public static void requestServerName() {
		SocketManager.sendCommand(Command.GET_SERVER);
	}
	...
	public static void kickPlayer(String player, String message) {
		SocketManager.sendCommand(Command.KICK_PLAYER, player, message);
	}
